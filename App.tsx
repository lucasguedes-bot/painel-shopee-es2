...
// Função auxiliar para transformar os registros do Supabase na estrutura do Dashboard
const processSupabaseData = (rows: any[]): FullData => {
  ...
  turnos.forEach((turno) => {
    const rowsTurno = rows.filter((r) => r.turno === turno);
    ...
    rowsTurno.forEach((row) => {
      const bips = Number(row.bips || 0);
      const ops = row.ops_id || row.operador || 'DESCONHECIDO';
      const nome = row.nome_colaborador || row.operador || 'DESCONHECIDO';
      
      // Combina o OPS e o Nome para exibição conjunta
      const labelOpsNome = ops !== 'DESCONHECIDO' && nome !== 'DESCONHECIDO' 
        ? `${ops} - ${nome}` 
        : ops !== 'DESCONHECIDO' ? ops : nome;

      const esteira = String(row.esteira || '').toLowerCase();
      const horaStr = row.hora || 'Sem Hora';

      if (esteira.includes('termo')) {
        totalTermo += bips;
        mapTermoOps.set(labelOpsNome, (mapTermoOps.get(labelOpsNome) || 0) + bips);
        mapTermoNomes.set(nome, (mapTermoNomes.get(nome) || 0) + bips);

        if (!mapHorasTermo.has(horaStr)) mapHorasTermo.set(horaStr, new Map());
        const hMap = mapHorasTermo.get(horaStr)!;
        hMap.set(labelOpsNome, (hMap.get(labelOpsNome) || 0) + bips);

      } else if (esteira.includes('lona')) {
        totalLona += bips;
        mapLonaOps.set(labelOpsNome, (mapLonaOps.get(labelOpsNome) || 0) + bips);
        mapLonaNomes.set(nome, (mapLonaNomes.get(nome) || 0) + bips);

        if (!mapHorasLona.has(horaStr)) mapHorasLona.set(horaStr, new Map());
        const hMap = mapHorasLona.get(horaStr)!;
        hMap.set(labelOpsNome, (hMap.get(labelOpsNome) || 0) + bips);

      } else if (esteira.includes('inbound')) {
        totalInbound += bips;
        if (!mapHorasInbound.has(horaStr)) mapHorasInbound.set(horaStr, new Map());
        const hMap = mapHorasInbound.get(horaStr)!;
        hMap.set(labelOpsNome, (hMap.get(labelOpsNome) || 0) + bips);
      }
    });
    ...
    const toHoraAtivaArray = (mapHora: Map<string, Map<string, number>>): HoraAtiva[] => {
      return Array.from(mapHora.entries()).map(([hora, mapOps]) => ({
        hora,
...
export default function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        const parsed = processSupabaseData(data);
        setFullData(parsed);
      } else {
        setFullData(null);
      }
  ...
  useEffect(() => {
    fetchData();
    const interval = setInterval(fetchData, 60000); // Atualiza a cada 1 min
  ...
  const renderTable = (list?: ItemRanking[], isHora = false, limit: number | null = null) => {
    ...
    const colHeader = isPublicTab ? 'OPS / COLABORADOR' : 'COLABORADOR';
    ...
    <div className={`app-container ${darkMode ? 'dark-theme' : ''}`}>
      ...
      {shiftData || currentEsteira === 'geral' || currentEsteira === 'gestores' ? (
        ...
        <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>
          <p style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>Nenhum dado encontrado para as opções selecionadas.</p>
          <p style={{ fontSize: '0.9rem' }}>Verifique se existem registros no Supabase para o período de {periodLabel}.</p>
        </div>
      )}
